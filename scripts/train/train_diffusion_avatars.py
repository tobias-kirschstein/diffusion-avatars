...
from diffusion_avatars.config.diffusion_avatars import DiffusionAvatarsTrainConfig, DiffusionAvatarsOptimizerConfig, DiffusionAvatarsModelConfig
from diffusion_avatars.data_manager.rendering_data_manager import RenderingDataFolder, RenderingName
from diffusion_avatars.dataset.rendering_dataset import SegmentationType, DatasetSplit, RenderingDatasetConfig, TextureFieldConfig, RenderingDataset, \
    TemporalBatchRenderingDatasetView
from diffusion_avatars.model.diffusion_avatars import DiffusionAvatarsExperimentConfig, DiffusionAvatarsModel
from diffusion_avatars.model.temporal.temporal_config import PositionalEncodingType, VideoControlNetConfig
from diffusion_avatars.model_manager.diffusion_avatars_model_manager import DiffusionAvatarsModelManager, DiffusionAvatarsModelFolder
from diffusion_avatars.renderer.provider.mesh_provider import TemporalSmoothingConfig
from diffusion_avatars.util.log import log_to_wandb
...
def main(
        ...
        dataset_version: str,
        /,
        rendering_names: str = "depths,uv,normals,uv_ambient",
        resolution: int = 512,
        ...
        interpolate_target_images: bool = True,
        ...
        use_canonical_coordinates: bool = True,
        ...
        use_temporal_unet_encoder: bool = True,
        use_temporal_unet_decoder: bool = True,
        use_temporal_controlnet_encoder: bool = False,

        positional_encoding: Optional[PositionalEncodingType] = None,

        # Expression codes
        use_expression_condition: bool = True,
        smooth_expression_codes: bool = False,
        include_eye_condition: bool = True,
    ...
    experiment_config = DiffusionAvatarsExperimentConfig(
    ...
    model_manager: DiffusionAvatarsModelManager = DiffusionAvatarsModelFolder().new_run(train_config.name)
    ...
    diffusion_avatars_model = DiffusionAvatarsModel(model_config,
                                                    dataset_config,
                                                    train_config=train_config,
                                                    optimizer_config=optimizer_config)
    ...
    diffusion_avatars_model.change_inference_scheduler('ddpm')
    ...
    accelerator = diffusion_avatars_model.setup_accelerator(accelerator_project_config)
    ...
    optimizers_and_lr_schedulers = diffusion_avatars_model.configure_optimizers()
    ...
    summary(diffusion_avatars_model)
    diffusion_avatars_model.to(accelerator.device)
    ...
    latent_shape = diffusion_avatars_model.get_latent_shape()
    validation_latent_noise = torch.randn((train_config.n_validation_images,) + latent_shape,
                                          device=diffusion_avatars_model.device, generator=generator)

    is_first_validation = True
    for epoch in range(first_epoch, train_config.num_train_epochs):

        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(diffusion_avatars_model):
                # Autocast is only necessary if we add trainable layers into the SD backbone
                with accelerator.autocast():
                    loss_dict = diffusion_avatars_model.get_loss_dict(batch)
                loss = diffusion_avatars_model.combine_losses(loss_dict)
                accelerator.backward(loss)

                # Gradient clipping and gradient logging
                if accelerator.sync_gradients:
                    params_to_clip = diffusion_avatars_model.controlnet.parameters()
                    ...
                    controlnet_grad_norm = mean([p.grad.norm().item()
                                                 for p in diffusion_avatars_model.controlnet.parameters()
                                                 if p.requires_grad and p.grad is not None])
                    wandb.log({"grad_norm/controlnet": controlnet_grad_norm}, step=global_step)

                    unet_learnable_parameters = diffusion_avatars_model.unet.get_learnable_parameters()
                    ...
                    # Neural Textures
                    if diffusion_avatars_model.neural_texture_lookup is not None:
                        wandb.log({"grad_norm/neural_texture_lookup":
                                       diffusion_avatars_model.neural_texture_lookup.weight.grad.norm().item()}, step=global_step)

                    if diffusion_avatars_model.ambient_texture_lookup is not None:
                        wandb.log({"grad_norm/ambient_texture_lookup":
                                       diffusion_avatars_model.ambient_texture_lookup.weight.grad.norm().item()}, step=global_step)

                    if diffusion_avatars_model.neural_texture_fields is not None:
                        wandb.log({"grad_norm/neural_texture_fields":
                                       mean([neural_texture_field.params.grad.norm().item()
                                             for neural_texture_field in diffusion_avatars_model.neural_texture_fields
                                             if neural_texture_field.params.grad is not None])},
                                  step=global_step)

                    if diffusion_avatars_model.neural_texture_lookup_uv is not None:
                        wandb.log({"grad_norm/neural_texture_lookup_uv":
                                       diffusion_avatars_model.neural_texture_lookup_uv.weight.grad.norm().item()}, step=global_step)
                        wandb.log({"grad_norm/neural_texture_lookup_uw":
                                       diffusion_avatars_model.neural_texture_lookup_uw.weight.grad.norm().item()}, step=global_step)
                        wandb.log({"grad_norm/neural_texture_lookup_vw":
                                       diffusion_avatars_model.neural_texture_lookup_vw.weight.grad.norm().item()}, step=global_step)

                    # Background Texture
                    if diffusion_avatars_model.learnable_background_color is not None:
                        wandb.log({"grad_norm/background_texture":
                                       diffusion_avatars_model.learnable_background_color.grad.norm().item()}, step=global_step)
                    ...
                    if model_config.temporal_config.temporal_batch_size >= 1 and model_config.temporal_config.use_temporal_convolution:
                        temporal_convolutions_grad_norms = [p.grad.norm().item()
                                                            for n, p in diffusion_avatars_model.controlnet.named_parameters()
                                                            if p.requires_grad and p.grad is not None
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % train_config.save_model_every == 0:
                        model_manager.store_checkpoint(diffusion_avatars_model, global_step)
                        ...
                        with torch.no_grad():
                            with accelerator.autocast():
                                ...
                                for split, dataset in datasets.items():
                                    ...
                                    diffusion_avatars_model.prepare_batch(validation_batch)
                                    ...
                                    for small_validation_batch in tqdm(
                                        ...
                                        diffusion_avatars_model.prepare_autoregressive_inference(small_validation_batch)
                                        inference_output = diffusion_avatars_model.inference(small_validation_batch)
                                        inference_outputs.extend(inference_output)

                                        valid_loss_dict = diffusion_avatars_model.get_loss_dict(small_validation_batch)
